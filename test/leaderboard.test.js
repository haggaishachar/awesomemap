...
test("computeLeaderboard respects the limit", () => {
  const result = computeLeaderboard(DOMAINS, { scope: "global", windowDays: 7, limit: 2, now: NOW });
  assert.deepEqual(result.map((r) => r.id), ["c/c", "b/b"]);
});

test("computeLeaderboard scoped to one domain only ranks that domain's projects", () => {
  const result = computeLeaderboard(DOMAINS, { scope: "data-science", windowDays: 7, limit: 20, now: NOW });
